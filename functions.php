...
/**
 * Loop through category posts and display them in a grid layout.
 *
 * @param mixed  $category_name Category name
 * @param mixed  $all_posts
 * @param string $view_more_text
 * @param string $heading_border_position
 * @param int    $heading_border_width
 *
 * @return string
 */
function loop_category_posts(
	$category_name = null,
	$all_posts,
	$view_more_text = 'View All',
	$heading_border_position = 'left',
	$heading_border_width = 5
) {
	...
	if ( $category_name ) {
		...
		$html .= <<<HTML
			<div class="ncbd-block-category-heading">
				<h2 class="ncbd-block-title{$border_class}" style="{$border_style}">{$category_name}</h2>
				<a href="{$category_link_url}" class="ncbd-block-category-link">{$view_more_text_escaped}</a>
			</div>
		HTML;
	} else {
		$border_class = 'bottom' === $heading_border_position ? ' bottom-border' : '';
		...
		$html .= <<<HTML
			<div class="ncbd-block-category-heading">
				<h2 class="ncbd-block-title{$border_class}" style="{$border_style}">Latest Posts</h2>
		HTML;

		if ( ! is_archive() ) {
			$html .= <<<HTML
				<a href="{$blog_url}" class="ncbd-block-category-link">{$view_more_text_escaped}</a>
			HTML;
		}
				...
				<div class="ncbd-post">
					...
					<div class="ncbd-post-content">
						...
						<div style="flex: 1 1 100%;display:flex;justify-content: flex-end;flex-direction: column;">
							...
							<p style="margin:0;font-style:italic;color:#555;margin-top:5px;">{$post_time_diff} ago</p>
						</div>
					</div>
				</div>
			HTML;
...
function create_pages_if_not_exist() {
	$pages = array( 'Home', 'About', 'Services', 'Clients', 'Works', 'Contact', 'Product Redesign', 'MVP', 'Team Extention', 'Case Study', 'Blog' );
...
function render_post_item( $post, $signle = false ) {
	$post_thumbnail = get_the_post_thumbnail( $post->ID, '', array( 'style' => 'border-radius:8px;max-height:518px;object-fit:cover;width:100%' ) );
...
function render_testimonials_tabs( $attributes, $content ) {
	...
	<div class="newschannelbd-testimonials-tabs">
		...
		<div class="newschannelbd-testimonials-tab-content">
			<?php
			echo $content; // Render inner blocks content
			?>
		</div>
	</div>
	<?php
...
add_filter(
	...
	function ( $html, $post_id, $post_thumbnail_id, $size, $attr ) {
		...
		if ( $caption && is_singular() ) {
			$html .= '<div class="post-thumbnail-caption" style="text-align:center;font-size:0.95em;color:#666;margin-top:0.5em;">' . esc_html( $caption ) . '</div>';
		}
		return $html;
	},
	10,
